...
async def add_product(message: Message, state: FSMContext):
    if message.from_user.id != 674796107 or message.from_user.id != 6593479727:
        await message.bot.send_message(chat_id=message.from_user.id, text=f'{message.from_user.id} Недостаточно прав!', reply_markup=kb.menu)
    else:
...
async def save_user(message: Message, state: FSMContext):
    ...
    sync_result = po.sync_one_prod(sbis_id)
    if sync_result:
